...
Обычный режим (не перестановка) держит на каждой строке три взаимодействия + скролл: instant-highlight на touch-down, quick-tap с роутингом по X (чекмарк → toggle дня, левая зона группы → expand), press-and-hold 0.4s → edit-шит. Реализация — `RowGestureOverlay: UIViewRepresentable`, наложенный `.overlay`'ем поверх чисто-визуального `content`.
