...
|------|----------|--------|
| **Quick tap** (<0.3s) | по чекмарку → toggle `done/missed` того дня; по левой части → ничего | **вся строка** → expand/collapse (у группы нет per-day чекмарков → правая зона не мёртвая, наследует действие строки) |
...
> **⚠️ Архитектура с тех пор разделена на ДВА режима, и ОБА жеста ушли в UIKit.** Раздел `### Реализация жестов` ниже описывает прежний одно-режимный SwiftUI-дизайн (press=edit и drag=reorder на одном long-press) — он **исторический**. Инварианты `.global`/feedback-loop, «не Button внутри строки», кэш DateFormatter **всё ещё в силе**. Текущее состояние (iOS 26): обе подсистемы перестали полагаться на SwiftUI-жесты — приватный pan ScrollView'а нельзя скоординировать из SwiftUI, поэтому жесты владеются `UIGestureRecognizer`'ами через `UIGestureRecognizerRepresentable`. **Обычный режим** — `RowGestureOverlay` (tap по X + long-press-edit + instant-highlight), см. `## Обычный режим`. **Режим перестановки** (FAB-тогл «↑↓»→«✓») — `ReorderLongPressGesture` (whole-row drag), см. `## Перестановка`. Edit-форма — `## Edit/Add sheets`.
...
- **Navbar Habits, справа** — `doc.on.doc` (копировать недавний лог) + `trash` (очистить). Гейтятся тумблером Settings → Диагностика → «Показывать лог-кнопки» (App-Group ключ `habit.showLogButtons`, **отдельный** от Voice'ового `devMode` намеренно — вкладки независимы; **off по умолчанию**, navbar чистый). Зеркалит механику Voice-таба (там `devMode` гейтит свои лог-иконки).
...
## Обычный режим: tap/expand/edit на UIKit-overlay

Обычный режим (не перестановка) держит на каждой строке три взаимодействия + скролл: instant-highlight на touch-down, quick-tap с роутингом по X, press-and-hold 0.4s → edit-шит. Реализация — `RowGestureOverlay: UIViewRepresentable`, наложенный `.overlay`'ем поверх чисто-визуального `content`.

X-роутинг в `handleTap` асимметричен по типу строки: у **привычки** правая зона (≥ `hPadding+leftZoneWidth`) → toggle дня, левая → no-op; у **группы** — `toggle` на **всю ширину** (guard по `loc.x` снят). Причина: у группы нет per-day чекмарков, поэтому правая зона ничего не toggl'ит — делать её мёртвой значило бы заставлять целиться в заголовок. Инертная под-зона наследует основное действие строки, а не остаётся неотзывчивой.

**Почему НЕ `Button` + `.onLongPressGesture` + `.simultaneousGesture(SpatialTapGesture)`** (этот стек был — и глушил И тап, И long-press на iOS 26): `Button` ставит приватный `_ButtonGesture`, который держит touch-sequence до touch-up и в unified-арбитраже iOS 18 не даёт внешним tap/long-press завершиться — ни expand, ни edit не срабатывали (хотя скролл при этом жил). `Button` как контейнер несовместим с навешанными сверху SwiftUI-жестами. Диагноз — deep-research (Perplexity). Общий корень тот же, что в Перестановке (приватный pan + unified-арбитраж) — см. `## Перестановка::Graveyard`.

**Решение — три `UIGestureRecognizer` в ОДНОМ `UIView`** (один overlay, чтобы `require(toFail:)` гарантированно встал до появления вью):
- **highlight** — `UILongPressGestureRecognizer(minimumPressDuration: 0)`, `allowableMovement` огромный: подсветка на `.began`, гаснет на `.cancelled`, когда pan забрал жест (точное поведение `UITableViewCell`, заменило `Button`+`RowPressStyle`).
- **longPress 0.4s** → edit.
- **tap.`require(toFail: longPress)`** → роутинг по X; так hold не порождает ещё и tap.

Все возвращают `shouldRecognizeSimultaneouslyWith → true` для всего, что НЕ один из этих трёх (т.е. для pan) → скролл жив; между своими тремя одновременен только highlight. *Старый инвариант «не Button внутри строки» теперь выполняется буквально: Button убран совсем.*
...
**Жест — свой `UILongPressGestureRecognizer` (`ReorderLongPressGesture: UIGestureRecognizerRepresentable`, `minimumPressDuration: 0.25`), прицеплен к ScrollView через `.gesture`.** Делегат возвращает `shouldRecognizeSimultaneouslyWith → true` для pan-recognizer'а → быстрый свайп (не дотянул до 0.25s) скроллит, удержание хватает строку. Это финал ПОСЛЕ ТРЁХ провалившихся pure-SwiftUI попыток — диагноз из deep-research (claude.ai Research, WWDC24 «unified gesture model» session 10118 + forum 760035).

Скролл и reorder оба вертикальный drag по одной строке в **скроллируемом** списке → их ОБЯЗАТЕЛЬНО разделять. Глиф-ручка (спатиальное разделение) отвергнута юзером → остаётся временнóе. **Graveyard трёх pure-SwiftUI попыток — все убивали скролл, общий корень: SwiftUI ScrollView pan приватный, его нельзя скоординировать из SwiftUI-жеста:**
1. `.gesture(DragGesture(minDist:8))` + `.scrollDisabled` (instant-grab): child-drag через `.gesture` (default priority) заставляет pan **ждать его fail'а**, low-minDistance drag не fail'ит → pan мёртв.
2. `LongPressGesture(0.25).sequenced(before: DragGesture(minDist:0))`: `SequenceGesture` арм'ит ОБЕ фазы на touch-down → внутренний drag захватывает touch ДО оценки таймера → тот же мёртвый скролл.
3. `.simultaneousGesture(DragGesture(minDist:0))` с тайм-гейтом в коде (`held = value.time - start`, `moved`): на устройстве iOS 26 всё равно голодил pan. Research-вывод: SwiftUI-жест через ЛЮБОЙ модификатор не сосуществует детерминированно с приватным pan'ом.

**Рабочее решение — владеть распознавателем в UIKit.** `UIGestureRecognizerRepresentable` (iOS 18) оборачивает `UILongPressGestureRecognizer`; его делегат `gestureRecognizer(_:shouldRecognizeSimultaneouslyWith:)` возвращает `true` для pan → UIKit разруливает scroll-vs-reorder, как в нативном списке. SwiftUI-строки и весь `.offset` make-room движок сохранены — в UIKit ушёл только ИСТОЧНИК жеста. Post-drop lock'а нет (`endReorder()` синхронный, recognizer перевзводится мгновенно — серийность «перенёс-сходу-хватаю-следующий» сохранена, это и была исходная боль с нативным `.onMove`). `.scrollDisabled(draggingIndex != nil)` остаётся belt-and-braces (флип один раз на engage).

**Баг «при прокрутке берётся строка на +N выше пальца» (а сверху списка — верно).** Индекс строки сперва считался через `context.converter.convert(globalPoint:to:.named(contentSpace))`. Но `convert(globalPoint:to:)` маппит в пространство **предка** вью, к которой прицеплен recognizer, а именованное пространство висело на `LazyVStack` — **потомке** → конверсия молча игнорировала прокрутку: на прокрутке вниз на N строк индекс занижался на N. **Фикс:** ловим истинный глобальный top контента через `.onGeometryChange { $0.frame(in: .global).minY }` в `@State contentTopY` (уходит в минус по мере скролла) и считаем `idx = Int((globalTouchY − contentTopY) / rowHeight)` — scroll-корректно по построению. *Что пробовали:* `.coordinateSpace(name:)` + named-конверсия — не учитывала скролл (см. выше, named-пространство на потомке), отброшено.
...
- **Описание привычки — `Habit.notes: String?`**, поле под заголовком в `EditHabitSheet`, нативный `TextField("Описание", text:, axis: .vertical)` + `lineLimit(1...6)` (растёт по контенту, без `TextEditor`-костылей с фоном/плейсхолдером). Заголовок и описание в ОДНОЙ `Section` — нативный разделитель строки между ними и есть «прочерк». Визуальная иерархия инвертирована: заголовок демотирован (14pt `.secondary`), описание — основной текст (17pt белый); почему именно так — `methodology/переносимый-дизайн.md::Визуальный вес — по ценности контента`. **`notes` ОБЯЗАН быть Optional**: тот же Codable-приём миграции, что у Voice (`fact-voice-record.md::noteFlag`/`recordKind`) — синтезированный декодер для опционала делает `decodeIfPresent`, и привычки из JSON ДО появления поля декодятся как `notes == nil`; будь поле non-optional `String`, decode старого дампа кинул бы `keyNotFound` → весь `try? decode(StorageData)` вернул бы nil → **все привычки молча стёрлись бы**. Дубликат `Habit` в виджете синхронизирован (`fix-ios-stability.md §3`). Пустое/пробельное описание нормализуется в `nil` в `updateHabit`.
...
- **Relative (Относительно сегодня)**: сетка всегда показывает текущий день на **последней** позиции. Справа от «сегодня» будущих дней нет, слева — предыдущие 6 дней. Дизайн-решение: история читается слева направо и заканчивается текущим днём.
...
## `WeekDay.id` обязан быть стабильным (не свежий UUID)

`WeekDay: Identifiable` имеет `var id: String { key }`, где `key` — строка даты (`2026-06-06`). **Не** `let id = UUID()`.

Симптом: при разворачивании/сворачивании группы шапка с днями недели (`DaysHeaderView`) **мигала** — колонки дней мелькали out+in. Причина: `DateHelper.weekDates()` пересоздаёт массив `WeekDay` на каждом проходе `body`, а `body` шапки переисполняется при ЛЮБОЙ публикации `store` (`@EnvironmentObject`), включая `toggleGroupExpanded`, обёрнутый в `withAnimation(.easeInOut)`. С `id = UUID()` каждый пересчёт давал 7 НОВЫХ идентичностей → `ForEach` под активной транзакцией удалял старые 7 колонок и вставлял новые, проигрывая insert/remove transition = мигание.

Инвариант: **identity у Identifiable-модели, которую фабрика пересоздаёт на каждый render, обязана выводиться из стабильного поля данных, а не из свежего `UUID()`.** Иначе `ForEach`/diffing считает каждый рендер полной заменой коллекции. Дата (`key`) для дня недели уникальна и неизменна между пересчётами — она и есть естественный id. То же относится к любой будущей derived-модели (`WeekDay`-подобной), создаваемой внутри `body`/helper'а.
