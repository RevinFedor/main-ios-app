...
мобильном экране. Model/effort/think открываются compact dropdown'ами от самих chips, как
в основном Gemini/Voice chat, а не full bottom-sheet. Hide-keyboard и scroll-to-bottom
...
полноценная страница истории. В нём есть заголовок `AI Chat`, нативная settings-кнопка,
...
живут в drawer; refresh-кнопки в chat header/drawer нет, потому что перезагрузка — не
