...
Timeline открывается не sheet'ом снизу и не отдельным push-экраном, а правым drawer'ом
поверх terminal chat: свайп справа-налево или floating button открывает панель, свайп
слева-направо/`x` закрывает. Chat underneath не двигается (`movesMain=false`), иначе
пользователь видит ложную анимацию "страница чата перелистывается", хотя он всего лишь
инспектирует соседнюю timeline-панель. Timeline по умолчанию скроллится к низу после
первого layout-yield и после изменения числа событий: свежие события важнее верха истории.
Логи этой поверхности идут как `[timeline]` + общий `[drawer-geo] edge=trailing`.
...
через пересечение keyboard end frame с `window.bounds` и bounds hosted-view после
`convert(..., from: screen.coordinateSpace)`. Шрам: reader view может иметь другой frame,
а iOS 26 safe-area обновления приходят не в той фазе, из-за чего input оказывался под
клавиатурой или ехал с рассинхроном. Второй шрам: SwiftUI/NavigationStack иногда уже
поднимает hosted subtree над клавиатурой даже при outer `.ignoresSafeArea(.keyboard)`;
если поверх этого применить полный ручной lift, composer улетает почти к верхнему краю.
Поэтому `baseBottomInset` запоминается только пока keyboard hidden, а итоговый lift =
`keyboardHeight - baseBottomInset - autoLiftAlreadyApplied`, где `autoLift` считается как
зазор между bottom окна и bottom hosted-view.
...
`inWindow`, `inView`, `safeBase`, `height`, `autoLift`, `lift`, `curve`, выбранный `anim`, `willHide`,
...
Drawer не двигает основную поверхность (`movesMain=false`). Визуально выезжает только левая
панель, а справа остаётся sliver текущего chat/terminal/projects состояния; это overlay
выбора, не перелистывание страницы. В ранней версии main content ехал вправо и после release
создавал ощущение, что проекты/чат перелистываются сами по себе. Выбор `All chats`, chat row
или `Terminal` применяется только после settle в closed-state через deferred action: heavy
center surface не монтируется в те же кадры, где drawer закрывается. Переход из обычного
Chat в Terminal дополнительно ставит короткий placeholder на один `Task.yield`, чтобы старый
chat subtree успел teardown'иться до mount тяжёлого terminal root; иначе редкие tap/gesture
последовательности могли дать чёрный экран или вылет без понятного live-log.

Диагностические теги drawer обязательны: `[drawer-gesture]` объясняет, почему pan был принят
или отвергнут (`disabled`, `nav-can-go-back`, `not-horizontal`, `closed-fullscreen`);
`[drawer-geo]` пишет edge, `movesMain`, offset, drawer frame, bounds и dim; `[drawer-black]`
срабатывает только на подозрительную геометрию (dim есть, drawer почти не виден, frame
невалиден или visible width не совпадает с progress). Для редких "чёрных экранов" это
важнее среднего frame time: timings могут быть чистыми, но target view/frame неправильный.
...
## CrashTrail: last breadcrumbs после неожиданного выхода

`CrashBreadcrumbs` пишет в App Group короткий ring-buffer последних навигационных событий
(`root-tab`, `voice-surface`, `drawer-action`, `terminal-nav`, `chat-load`, scene phase) и
ставит флаг `running`. При следующем cold start, если предыдущий запуск закончился не в
`background`, клиент один раз логирует `[CrashTrail] previous run ended unexpectedly ... last=[...]`.
Это не заменяет crash report, но закрывает мобильный сценарий, где приложение просто вылетело
после последовательности "terminal → chat → terminal", а live-log уже потерян.

В breadcrumbs не писать шум на каждый scroll/tick; нужны только границы ownership и surface
swap: кто владел экраном, был ли drawer открыт, какой project/tab выбран, была ли активна
клавиатура/terminal back-navigation. Если `CrashTrail` заканчивается на `drawer-action begin`
без соответствующего `end`, смотреть deferred surface swap; если на `terminalMode=true`
с открытым drawer — смотреть terminal mount placeholder; если на `terminal-nav backToTabs`
или `selectTab` — смотреть UIKit shell/chat teardown.
