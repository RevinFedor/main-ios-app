...
Второй виток этого же бага: в некоторых `NavigationStack`/hosted-controller состояниях
SwiftUI уже поднимает весь hosted subtree над клавиатурой, несмотря на outer
`.ignoresSafeArea(.keyboard)`. Если затем применить полный ручной lift, composer улетает к
верхнему краю экрана. Фикс — измерять hosted view в window coordinates и вычитать уже
применённый `autoLift` из ручного подъёма; terminal composer делает тот же расчёт и
reconcile'ит его при изменении высоты host-view.
Логи `[Keyboard]` должны писать `frameEnd/inWindow/inView/safeBase/height/autoLift/lift/curve/anim/willHide`,
`collapse reason=...` и отдельный `input tap`, иначе нельзя отличить геометрию, timing,
double-lift и перехват touch. Конкретная реализация и UX-модель — `fact-voice-chat-tab.md::Composer keyboard`.
...
### 10. History/Timeline drawer: overlay, deferred surface swap и black-screen diagnostics

Соседний шрам той же семьи, но уже не про `projects/tabs/chat` push. История чатов, All Chats,
Terminal toggle и Timeline тоже выглядят как "просто боковая панель", но если main surface
двигается или тяжёлый center surface монтируется в те же кадры, где drawer закрывается, юзер
видит дёрганье без плохих frame timings: справа остаётся не та часть проектов, чат анимируется
под timeline, или вместо панели на 1-2 жеста появляется чёрный экран. В логах при этом может
быть `worst≈17-30ms`, потому что проблема не в скорости кадра, а в том, какой view/frame
назначен target'ом в конкретной фазе.

Финальная модель для leading history drawer и trailing timeline drawer — один
`SideDrawerUIKitController`, где двигается только drawer (`movesMain=false`), а main page
остаётся статичной. Выбор `All chats`/chat/Terminal не меняет central surface до тех пор,
пока drawer не settled в closed-state; переход Chat → Terminal дополнительно показывает
пустой page background на один `Task.yield`, чтобы chat teardown закончился до mount тяжёлого
Terminal root. Timeline на terminal chat использует тот же контейнер справа: открывается
leftward pan'ом, не двигает chat underneath, скроллится к низу после layout-yield.

Что не повторять: не возвращать "open drawer then immediately switch surface behind it" и не
анимировать main page ради ощущения перелистывания. Это смешивает две модели — navigation
push и overlay drawer — и рождает ложные симптомы: пользователь думает, что страница проекта
уезжает, хотя он хотел открыть боковую историю. Для редких black-screen случаев нужны
геометрические логи `[drawer-gesture]`, `[drawer-geo]`, `[drawer-black]` и `CrashTrail`, а не
только `FrameMonitor`: если dim виден, а drawer frame/visible width не совпадает с progress,
это bug target geometry, даже если frame budget чистый.

Постоянный поток `[TerminalScroll]` после фикса держать выключенным. Он полезен только для
короткой сессии воспроизведения scroll-регрессии (anchor restore, distFromBottom, pin/yield),
но в обычной работе забивает лог и маскирует редкие drawer/crash события. В коде такие строки
идут через `CTVerboseScrollLogging=false`; включать флаг временно и выключать сразу после
снятия нужного trace.
